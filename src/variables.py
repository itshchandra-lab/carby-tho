...
def construct_net_energy_position(df: pd.DataFrame) -> pd.DataFrame:
    ...
    world_prod = df.groupby('year')['total_production_twh'].transform('sum').replace(0, np.nan)
    ...
    for era_name, weights in era_weights_map.items():
        ...
        for fuel, weight in weights.items():
            if fuel not in df.columns:
                continue
            world_fuel = df.groupby('year')[fuel].transform('sum').replace(0, np.nan)
...
def construct_thorium_monetary_potential(df: pd.DataFrame,
    ...
    if governance_df is not None:
        gov_merge = governance_df[['country_code', 'year'] +
                                   [c for c in gov_cols if c in governance_df.columns]].copy()
        df = df.merge(gov_merge, on=['country_code', 'year'], how='left')

    available_gov = [c for c in gov_cols if c in df.columns]
    if available_gov:
        # Normalise each to 0-1 range then average
        for col in available_gov:
            col_min = df[col].min()
            col_max = df[col].max()
            if col_max > col_min:
                ...
                df[f'{col}_norm'] = 0.5
        norm_cols = [f'{c}_norm' for c in available_gov]
        df['institutional_quality'] = df[norm_cols].mean(axis=1)
    else:
        df['institutional_quality'] = 0.5  # neutral if unavailable
        print("Warning: governance data unavailable. "
              "institutional_quality set to 0.5 (neutral).")
...
def construct_gdp_share(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    world_gdp = df.groupby('year')['gdp_usd'].transform('sum').replace(0, np.nan)
